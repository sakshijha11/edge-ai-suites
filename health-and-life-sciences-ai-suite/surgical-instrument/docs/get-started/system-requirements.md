...
| Intel client GPU stack (Level Zero + OpenCL + iHD) | `libze1`, `libze-intel-gpu1`, `intel-igc-core-2`, `libigdgmm12`, `intel-opencl-icd`, `intel-media-va-driver-non-free`. Installed by `./scripts/setup_prerequisites.sh`; verified by `make check-l0`. |
| Host groups `render` and `video` exist | The Makefile auto-detects the GIDs. `./scripts/setup_prerequisites.sh` adds the current user if missing. |
...
| Requirement | Notes |
|---|---|
| Python 3.10–3.12 with `python3-venv` | Host-side training venv (`.venv-backend`). Installed by `./scripts/setup_prerequisites.sh`. |
| REAL-Colon dataset subset (~74 GB) | Downloaded via `./scripts/download_realcolon_subset.sh` from figshare article `22202866`. Full 60-study, ~880 GB corpus available via `datasets/REAL-Colon/helper/download_dataset.sh`. |
