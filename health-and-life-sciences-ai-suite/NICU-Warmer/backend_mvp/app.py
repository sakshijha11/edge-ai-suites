...
def create_app(config_path: str) -> Flask:
    ...
    def start() -> Response:
        ...
        try:
            ...
            if not ok:
                # NPU fallback: if any device is NPU and error looks NPU-related, retry with CPU
                _uc_devs = backend._user_config.get("devices", {})
                npu_keys = [k for k, v in _uc_devs.items() if v == "NPU"]
                npu_keywords = ("npu", "accel", "device not found", "unsupported device")
                if npu_keys and any(kw in message.lower() for kw in npu_keywords):
                    logger.warning(
    ...
    def config_apply() -> Response:
        ...
        state = backend.lifecycle.state()
        if state in {Lifecycle.STARTING, Lifecycle.RUNNING}:
