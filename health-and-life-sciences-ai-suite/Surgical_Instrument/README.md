...
- **Pipeline Performance table** — `Workload | Model | Device | FPS | Infer mean | P90 | P95 | Status`. `Infer mean` is the per-frame model latency average; `P90` and `P95` are nearest-rank percentiles over the rolling recent-frame window.
...
#    (preferred mirror: https://www.kaggle.com/datasets/longvil/cvc-colondb ;
#     fallback: CVC lab download with research-use terms)
...
`make up` and `make run` both bootstrap `.env` from `.env.example` (if missing) and auto-detect any `/dev/video*` devices on
